...
uint16_t AX25Frame_HDLC_Parser(ax25frame_t* ax25frame , uint8_t* stuffedFrame, uint16_t stuffedFrameLen)
{
  ...
  uint8_t* frameBuff = (uint8_t*)malloc(frameBuffLen);
  ...
  for(size_t i = 0; i < frameBuffLen * 8; i++)
  {
    ...
    {
      // copy 0 and reset counter
      CLEAR_BIT_IN_ARRAY(frameBuff, i);
      stuffedFrameBuffLenBits++;
      count = 0;
    }
  }
  ...
  memcpy(ax25frameBuff, frameBuff, frameBuffLen);
  free(frameBuff);
  // verify CRC result
...
//    ax25frame->ax25RcvFrame->isCrcOk = true;
  ...
  memcpy(ax25frame->ax25RcvFrame->info, frameBuffPtr, ax25frameBuffLen - ((2*(RADIOLIB_AX25_MAX_CALLSIGN_LEN + 1)) + 1 + 1));
